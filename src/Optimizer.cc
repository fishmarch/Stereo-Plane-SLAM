...
int Optimizer::PoseOptimization(Frame *pFrame, float angleInfo, float disInfo, float planeChi)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    if(nInitialCorrespondences<3)
        return 0;
//Set Plane vertices
    const int M = pFrame->mvPlanes.size();
    ...
    const float deltaPlane = sqrt(planeChi);
    {
        ...
        for (int i = 0; i < M; ++i) {
            ...
            if(pMP){
                ...
                e->setMeasurement(Converter::toPlane3D(pFrame->ComputeCameraPlane(i)));
                Eigen::Matrix3d Info;
                Info << angleInfo, 0, 0,
                        0, angleInfo, 0,
                        0, 0, disInfo;
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, float angleInfo, float disInfo, float planeChi)
{    
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin() , lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        for (int i = 0; i < (*lit)->mvPlanes.size(); ++i) {
    ...
    for (list<MapPlane *>::iterator lit = lLocalMapPlanes.begin(), lend = lLocalMapPlanes.end();
        ...
        for (map<KeyFrame *, int>::const_iterator mit = observations.begin(), mend = observations.end();
            ...
            if (!pKFi->isBad()) {
                ...
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(Converter::toPlane3D(pKFi->ComputeCameraPlane(mit->second)));
                Eigen::Matrix3d Info;
                Info << angleInfo, 0, 0,
                        0, angleInfo, 0,
                        0, 0, disInfo;
