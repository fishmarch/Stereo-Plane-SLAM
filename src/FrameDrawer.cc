...
cv::Mat FrameDrawer::DrawFrame()
{
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(auto line : mvCurrentKeyLines){
            cv::line(im, line.getStartPoint(), line.getEndPoint(), cv::Scalar(0, 0, 255), 1);
        }

        float step = 0.01;
        for(int i = 0; i < mvPlanes.size(); i++){
            cv::Point3d planeNormal = mvPlaneNormal[i];
            int li = mvPlaneLineNo[i].first;
            int lj = mvPlaneLineNo[i].second;
            cv::Point3d directioni = mvLine3DDirections[li];
            cv::Point3d directionj = mvLine3DDirections[lj];
            cv::Point3d endpointi = mvLineEndPoints[li].first;
            cv::Point3d endpointi2 = mvLineEndPoints[li].second;
            cv::Point3d endpointj = mvLineEndPoints[lj].first;
            cv::Point3d endpointj2 = mvLineEndPoints[lj].second;
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    mvCurrentKeyLines=pTracker->mCurrentFrame.mvKeys_Line;
    ...
    mvLineEndPoints = pTracker->mCurrentFrame.mvLineEndPoints;
    mvLine3DDirections = pTracker->mCurrentFrame.mvLine3DDirections;
